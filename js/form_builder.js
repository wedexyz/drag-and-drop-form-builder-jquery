    ...
    function getPreview(plain_html = '') {
        ...
        el.each(function () {
            ...
            if (data_type === 'number') {
                ...
                html += '<div class="form-group" id="get"><label class="control-label">' + label + '</label><input type="number" name="' + name + '" placeholder="' + placeholder + '" class="form-control" ' + required + '/></div>';
            }
            if (data_type === 'email') {
                var placeholder = $(this).find('.form_input_placeholder').val();
                var checkbox = $(this).find('.form-check-input');
